...
A lightweight Tampermonkey userscript that automatically switches Google Gemini's mode picker to `Pro` or `Advanced` the moment the page loads. 
...
- **SPA Support**: Seamlessly detects internal navigation in Gemini.

## Getting Started

1. Make sure you have the [Tampermonkey](https://www.tampermonkey.net/) extension installed in your browser.
2. Ensure your Google account actually has access to Gemini `Pro` or `Advanced`.
3. Open the raw version of [`Default-GeminiPro.user.js`](./Default-GeminiPro.user.js) in your browser.
...
This is a pure frontend DOM-manipulation script. It aggressively polls for the mode picker on initial load, makes the switch, and then immediately stops checking until a new chat or page is loaded.
